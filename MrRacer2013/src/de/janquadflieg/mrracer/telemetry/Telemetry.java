...
public class Telemetry
    ...
    /** Information about which data entries belong to which lap. */
    private ArrayList<java.awt.Point> lapData = new ArrayList<>();
    /** The last lap time. */
    ...
    private final ArrayBlockingQueue<DataEntry> queue = new ArrayBlockingQueue<>(500);
    ...
    /** List of data sets. */
    private final ArrayList<DataEntry> data = new ArrayList<>(INITIAL_CAPACITY);
    /** The list of registered listeners. */
    private final ArrayList<TelemetryListener> listeners = new ArrayList<>();
    ...
    public int getNumLaps() {
        return this.lapData.size();
    }

    public int getLapStartIndex(int lap) {
        return lapData.get(lap).x;
    }

    public int getLapEndIndex(int lap) {
        return lapData.get(lap).y;
    }

    public double getTrackLength() {
        return approxTrackLength;
    }
    ...
    private void addData(DataEntry entry, boolean notify) {
        DataEntry lastEntry = null;

        if (data.isEmpty()) {
            java.awt.Point newLap = new java.awt.Point(0, 0);
            lapData.add(newLap);
        } else {
        ...
        if (lastEntry != null) {
            ...
            if (entry.s.getDistanceFromStartLine() < lastEntry.s.getDistanceFromStartLine()
                ...
                approxTrackLength = lastEntry.s.getDistanceFromStartLine()
                ...
                java.awt.Point newLap = new java.awt.Point(start, end);
                lapData.add(newLap);
                ...
                // update lap data
                lapData.get(lapData.size() - 1).y = data.size() - 1;
            }
        }
    ...
    public void log(SensorData data, Action a, String info) {
