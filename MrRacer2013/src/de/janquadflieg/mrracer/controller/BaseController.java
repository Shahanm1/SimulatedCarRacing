...
public abstract class BaseController
    ...
    protected DefensiveFallbackBehaviour backupBehaviour;
    ...
    public void paint(String baseFileName){
        
    }

    @Override
    public final scr.Action control(scr.SensorModel model) {
        try {
            return unsaveControl(model);

        } catch (RuntimeException e) {
            if (TEXT_DEBUG && !precompile) {
    ...
    public abstract scr.Action unsaveControl(scr.SensorModel model);
    ...
    /**
     * Method which executes several gameticks. This can be called in the constructor
    ...
    protected void precompile() {
        ...
        resetFull();
        control(new scr.MessageBasedSensorModel(PC_WARMUP_FIRST));
        // second packet
        setStage(Stage.WARMUP);
        resetFull();
        control(new scr.MessageBasedSensorModel(PC_WARMUP_SECOND));

        // offtrack
        setStage(Stage.WARMUP);
        resetFull();
        control(new scr.MessageBasedSensorModel(PC_WARMUP_OFFTRACK));
        resetFull();

        // ontrack
        setStage(Stage.WARMUP);
        resetFull();
        control(new scr.MessageBasedSensorModel(PC_WARMUP_ONTRACK));
        resetFull();
        ...
        for (int i = 0; i < data.size() / 2; ++i) {
            ...
            scr.Action a = control(wnoise.getSensorModel());
        }
        ...
        data.shutdown();
        setTrackName(TrackDB.PRECOMPILE_TRACK);
        setStage(Stage.RACE);
        resetFull();
