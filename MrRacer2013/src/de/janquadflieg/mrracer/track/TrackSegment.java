...
public class TrackSegment
    ...
    /** Sub segments. */
    private ArrayList<TrackSubSegment> segments = new ArrayList<>();
    /** Filtered? */
    ...
    public static final double DEFAULT_BCC = Double.MIN_VALUE;
    public static final double DEFAULT_PCF = Double.MIN_VALUE;
    private double[] targetSpeeds = null;
    private transient Apex[] apexes = null;
    /** The bcc factor for this segment. */
    private double bcc = DEFAULT_BCC;
    /** The pcf factor for this segment. */
    private double pcf = DEFAULT_PCF;
    ...
    public Apex[] getApexes() {
        if (apexes == null) {
            apexes = calculateApexes();
        }
        return apexes;
    }   

    public void setBCC(int i, double d){
        bcc = d;
        apexes[i].bcc = d;
    }
    
    public void setPCF(int i, double d){
        pcf = d;
        apexes[i].pcf = d;
    }

    /**
     * Method used by the track model only!
     * @return
     */
    protected Apex[] calculateAndGetApexes() {
        this.apexes = calculateApexes();
        return apexes;
    }

    private Apex[] calculateApexes() {
        if (unknown) {
            Apex apex = new Apex();
        ...
        ArrayList<Apex> list = new ArrayList<>();
        for (int i = 2; i < segments.size(); ++i) {
    ...
    protected void initTargetSpeeds() {
        TrackSegment.Apex[] myapexes = calculateAndGetApexes();
        targetSpeeds = new double[myapexes.length];
    ...
    public void write(java.io.OutputStreamWriter w, int version)
        ...
        w.write(" ");
        if(version >=4){
            w.write(String.valueOf(bcc));
            w.write(" ");
            w.write(String.valueOf(pcf));
            w.write(" ");
        }
    ...
    public static TrackSegment read(String s, int version, boolean modelComplete)
        ...
        if(version >=4){
            result.bcc = Double.parseDouble(tokenizer.nextToken());
            result.pcf = Double.parseDouble(tokenizer.nextToken());            
        } else {
            result.bcc = TrackSegment.DEFAULT_BCC;
            result.pcf = TrackSegment.DEFAULT_PCF;
        }
        ...
        if (modelComplete) {
            ...
            if (version > 2 && tokenizer.hasMoreTokens()) {
                for (int i = 0; i < result.targetSpeeds.length; ++i) {
                    result.setTargetSpeed(i, Double.parseDouble(tokenizer.nextToken()));
                }
            }
        }
    ...
    public String toString(boolean linebreak) {
        if (unknown) {
            ...
            if (isCorner()) {
                TrackSegment.Apex[] myapexes = getApexes();
                for (TrackSegment.Apex a : myapexes) {
                    result += "\n   ";
                    result += a.toString();
                }
            }
    ...
    public static class Apex {
        ...
        public double bcc = TrackSegment.DEFAULT_BCC;
        public double pcf = TrackSegment.DEFAULT_PCF;
    ...
    public static class Painter {
        ...
        /** Flag. */
        public static final int DRAW_SUBSEGMENTS = 0x1;
        /** Flag. */
        public static final int DRAW_START_END = 0x2;
        /** Height in pixel of the segment block. */
        private static final int BLOCK_HEIGHT = 100;

        public static java.awt.Dimension draw(TrackSegment seg, Graphics2D g, double ppm, int flags) {
            int width = (int) Math.round(seg.getLength() * ppm);
            //width = Math.max(1, width);

            boolean drawSS = (flags & DRAW_SUBSEGMENTS) == DRAW_SUBSEGMENTS;
            boolean drawStartEnd = (flags & DRAW_START_END) == DRAW_START_END;

            Dimension result = new Dimension(width, BLOCK_HEIGHT);
            ...
            g.fillRect(0, 0, width, BLOCK_HEIGHT);

            if(drawStartEnd){
                g.setColor(Color.BLACK);
                String s = Utils.dTS(seg.getStart());
                g.drawString(s, 5, BLOCK_HEIGHT+g.getFontMetrics().getHeight());

                s = Utils.dTS(seg.getEnd());
                g.drawString(s, width-(g.getFontMetrics().stringWidth(s)+5),
                        BLOCK_HEIGHT+g.getFontMetrics().getHeight());
...
//                g.drawString(Utils.dTS(model.getSegment(nIdx).getEnd()), size.width - 100, 115);
            ...
            if (drawSS && !seg.isUnknown()) {
                ...
                for (int i = 0; i < seg.size(); ++i) {
                    ...
                    g.fillRect(0, 4, width, BLOCK_HEIGHT-8);
