 ...
 * @author quad
 */
public class OnlineLearning2012 {
    ...
    enum LearningPhase {
        ...
        TrackModel, Waiting, Testing, GoingDown, GoingUp, GoingFaster
    };
    /** The current phase. */
    private LearningPhase phase = LearningPhase.Waiting;

    public OnlineLearning2012() {
...
//            }
    ...
    private void updateGoingDown(boolean justSwitched, int bestLap) {
        ...
        if (toAnalyze.isOk()) {
            ...
            if (secondToLastLap.getPhase() == LearningPhase.GoingDown
                ...
                if (TEXT_DEBUG) {
                    System.out.println("I completed two laps without problems, switching to going up...");
                }
    ...
    private void updateGoingUp(boolean justSwitched) {
        if (justSwitched) {
            ...
            LapData oldLap = lapData.get(lapData.size() - 1);
            LapData newLap = new LapData(lapData.size() + 1, LearningPhase.GoingUp, lapData.get(lapData.size() - 1));
            lapData.add(newLap);

            for (int i = 0; i < oldLap.size(); ++i) {
                DataSet ds = oldLap.get(i);
                DataSet newDs = newLap.get(i);
                ...
                for (int k = 0; k < ds.speeds.length; ++k) {
                    newDs.speeds[k] = ds.speeds[k] * (1.0 + ds.plusCorrection);
                    if (TEXT_DEBUG) {
                        System.out.println("Setting speed from " + Utils.dTS(ds.speeds[k]) + " to " + Utils.dTS(newDs.speeds[k]));
                    }
                    seg.setTargetSpeed(k, newDs.speeds[k]);
                }
            }
        ...
        if (lastLap.isOk()) {
            ...
            if (TEXT_DEBUG) {
                System.out.println("Last lap was not ok");
            }
            int triesToGoUp = 0;
            for (int i = 0; i < lapData.size(); ++i) {
                LapData ld = lapData.get(i);
                if (ld.phase == LearningPhase.GoingUp && !ld.isFlying()) {
                    ++triesToGoUp;
                }
            }
            ...
            if (triesToGoUp == 2) {
                if (TEXT_DEBUG) {
                    System.out.println("I tried faster speeds two times without success, switching to going faster (trying to get the other corners faster");
                }
                phase = LearningPhase.GoingFaster;
                int referenceLap = 0;
                for (int i = 0; i < lapData.size(); ++i) {
                    ...
                    if (ld.getPhase() == LearningPhase.GoingDown) {
                        referenceLap = ld.getLap();
                    }
                }
                updateGoingFaster(true, referenceLap);
                return;
            }
        }

        // last lap was not ok, let's give it another try
        if (TEXT_DEBUG) {
            System.out.println("Adjusting speeds further down");
        }
        ...
        for (int i = 0; i < lastLap.size(); ++i) {
            ...
            if (TEXT_DEBUG) {
                System.out.println("[" + i + "] latSpeedInt: " + Utils.dTS(ds.latSpeedInt)
                        + ", max: " + Utils.dTS(ds.maxLatSpeed));
            }
            ...
            if (ds.offTrack) {
                ...
                for (int k = 0; k < ds.speeds.length; ++k) {
                    newDs.speeds[k] = ds.speeds[k] / (1.0 + ds.plusCorrection);
                    newDs.plusCorrection = 0.5 * ds.plusCorrection;
                    newDs.speeds[k] = newDs.speeds[k] * (1.0 + newDs.plusCorrection);
                    if (TEXT_DEBUG) {
                        System.out.println("Setting speed from " + Utils.dTS(ds.speeds[k]) + " to " + Utils.dTS(newDs.speeds[k]));
                    }
                    seg.setTargetSpeed(k, newDs.speeds[k]);
    ...
    public void saveData(String suffix) {
        ...
        if (best == null) {
            System.out.println("No candidate found, taking a look at phase " + LearningPhase.GoingUp.toString());
            best = getBestLap(LearningPhase.GoingUp);
        }

        if (best == null) {
            System.out.println("Still no candidate found, taking a look at phase " + LearningPhase.GoingDown.toString());
        ...
        time = 0.0;
        for (LapData ld : lapData) {
            if (ld.getPhase() == LearningPhase.GoingUp) {
                time += ld.getTime();
            }
        }
        System.out.println("Correcting speeds up again took " + Utils.timeToExactString(time));
    ...
    static class LapData extends ArrayList<DataSet> {
        ...
        private OnlineLearning2012.LearningPhase phase;
        ...
        public LapData(int lap) {
            this.lap = lap;
            this.phase = OnlineLearning2012.LearningPhase.Waiting;
        }

        public LapData(int lap, double time) {
            this.lap = lap;
            this.phase = OnlineLearning2012.LearningPhase.TrackModel;
            this.time = time;
        }

        public LapData(int lap, OnlineLearning2012.LearningPhase phase, LapData a) {
            this.lap = lap;
            this.phase = phase;
        ...
        public LapData(TrackModel model, int lap, OnlineLearning2012.LearningPhase phase,
                Plan plan) {
            super(model.size());
            this.lap = lap;
            this.phase = phase;
        ...
        public OnlineLearning2012.LearningPhase getPhase() {
            return phase;
        }
