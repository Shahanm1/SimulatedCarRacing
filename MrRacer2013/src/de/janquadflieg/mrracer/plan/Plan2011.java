...
public class Plan2011
    ...
    public Plan2011(BaseController c) {
        this.observer = new Observer2013();
        this.moduleStraight = new PlanStraight(this);
        this.moduleCorner = new PlanCorner2012Flexible(this);
        this.controller = c;
        if (GRAPHICAL_DEBUG) {
    ...
    public void setParameters(Properties params, String prefix) {
        ...
        brakePredictor.setParameters(params, prefix + Plan2011.BRAKE_PREDICTOR);
        moduleCorner.setParameters(params, prefix + Plan2011.MODULE_CORNER);
        moduleStraight.setParameters(params, prefix + Plan2011.MODULE_STRAIGHT);
        mu = Double.parseDouble(params.getProperty(prefix + Plan2011.MU, String.valueOf(mu)));
        brakeCornerCoeff = Double.parseDouble(params.getProperty(prefix + Plan2011.BRAKE_CORNER_COEFF, String.valueOf(brakeCornerCoeff)));
    }
    ...
    public void getParameters(Properties params, String prefix) {
        ...
        brakePredictor.getParameters(params, prefix + Plan2011.BRAKE_PREDICTOR);
        moduleCorner.getParameters(params, prefix + Plan2011.MODULE_CORNER);
        moduleStraight.getParameters(params, prefix + Plan2011.MODULE_STRAIGHT);
        params.setProperty(prefix + Plan2011.BRAKE_CORNER_COEFF, String.valueOf(brakeCornerCoeff));
        params.setProperty(prefix + Plan2011.MU, String.valueOf(mu));
    }
    ...
    public final void calcApproachSpeed(PlanStackData planData) {
        ...
        dummyData.setSpeed(Plan2011.MAX_SPEED);
        ...
        do {
            ...
            dummyPlanData.addSegment(index, segment.getLength(), Plan2011.MAX_SPEED);
    ...
    public double getAnchorPoint(TrackSegment first, TrackSegment second) {
        return getAnchorPointImpl(first, second);
    }

    private double getAnchorPointImpl(TrackSegment first, TrackSegment second) {
    ...
    public static void main(String[] args) {
        Plan p = new Plan2011(new de.janquadflieg.mrracer.controller.MrRacer2012());
    ...
    private class DebugPainter
        ...
        public void paintComponent(Graphics2D g) {
            ArrayList<PlanElement2011> plan = new ArrayList<>(Plan2011.this.plan);
            ...
            for (int i = 0; i < plan.size(); ++i) {
                ...
                if (Plan2011.this.trackModel.initialized()) {
                    double d1 = Plan2011.this.toDistanceFromStartLine(Math.min(
                            (plan.get(i).getStart() + plan.get(i).getEnd()) / 2.0,
                            plan.get(i).getStart() + 0.5));
                    TrackSegment s1 = Plan2011.this.trackModel.getSegment(d1);

                    double d2 = Plan2011.this.toDistanceFromStartLine(Math.max(
                            (plan.get(i).getStart() + plan.get(i).getEnd()) / 2.0,
                            plan.get(i).getEnd() - 0.5));
                    TrackSegment s2 = Plan2011.this.trackModel.getSegment(d2);
